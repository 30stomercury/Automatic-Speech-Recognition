...
class BeamState(object):
    ...
    def __init__(self, token_ids, log_prob, dec_state, lm_state):
        """hypothesis
        Args:
            token_id: Index of the starting tokenactor, 1 corresponding to <SOS>.       
            log_prob: Log probability of the starting tokenactor.
    ...
    def update(self, token_id, log_prob, dec_state, lm_state):
...
class BeamSearch(object):
    ...
    def decode(self, sess, xs):
        ...
        beam_set = [BeamState([self.start_id], 0, dec_init_state, lm_init_state)]*self.beam_size 
        selected_beam_state = []
        while t < dec_step and len(selected_beam_state) < self.beam_size:
            ...
            # decoding
            logits, dec_states = self._get_decode(
                                    sess, h, enc_len, prev_token_ids, prev_dec_states)
            if self.args.apply_lm:
                prev_lm_states = [b.lm_state for b in beam_set]
                prev_lm_states = np.split(
                                np.concatenate(prev_lm_states, 2).reshape([-1, 512]), 4)
            ...
            for i in range(num_beam):
                topk_ids = np.argsort(logits[i])[4:]       # => argsort is in acending order
                topk_probs = logits[i][topk_ids]
                for j in range(len(topk_ids)):  # => search each node
                    if t > 0 and topk_ids[j] == self.token_to_id["<SOS>"]:
                        continue

                    if self.args.apply_lm:
                        beam_set_bank.append(beam_set[i].update(
                                            topk_ids[j], topk_probs[j], 
    ...
    def _build_decode(self):
        ...
        self.cur_token, self.dec_state, self.alphas = self.speller.decode(
                            self.h, self.h_len, rnn_tuple_state, prev_token, is_training=False)
    ...
    def _get_decode(self, sess, enc_out, enc_len, prev_token_id, dec_state_packed):
        """get decoder output"""
        N = len(prev_token_id)
        feed_dict = {
    ...
    def restore_las(self, sess, save_path, restore_epoch):
        ...
        var_las = self._get_decode_varlist(ckpt) # TODO
        # restore
